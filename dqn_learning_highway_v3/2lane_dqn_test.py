...
def update_acc(output):
    global cumulative_acc, xNum_acceleration
    cumulative_acc.append(output['vacc'])
    if len(xNum_acceleration) <= NUM_ACC + 1:
        xNum_acceleration.append(output['vacc'])
...
def save_metrics():
    ...
    with open(metrics_file, 'w', newline ='') as f:
        ...
        writer.writerows(zip(cum_collision_rates,ep_avg_spd, ep_std_dev, ep_avg_acc,ep_acc_std_dev,collisions))
    metrics_file = "data/{}-{}".format("acceleration", VERSION)+".csv"
    with open(metrics_file, 'w', newline ='') as f:
        writer = csv.writer(f)
        # writer.writerows(map(lambda x: [x], cum_collision_rates))
        writer.writerows(zip(xNum_acceleration))
