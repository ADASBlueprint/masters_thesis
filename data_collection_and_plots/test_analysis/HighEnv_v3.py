...
ACTION_SPACE_SIZE = 5
output_keys = ['va', 'v1', 'd1', 'v2', 'd2', 'v3', 'd3', 'v4', 'd4', 'v5', 'd5', 'v6', 'd6', 'v12', 'd12', 'v32', 'd32', 'v52', 'd52','L', 'vacc']
output_values = [0, 0, max_dist, 0, max_dist, 0, max_dist,0, max_dist, 0, max_dist, 0, max_dist, 0, max_dist, 0, max_dist, 0, max_dist, 0, 0]   
...
#        traci.vehicle.deactivateGapControl("Auto")
    ...
    def target_assignment(self, veh, v_key, d_key):
        d1 = abs(veh[1][tc.VAR_POSITION][0] - self.auto_pos)
        if abs(d1) < max_dist:
            self.output[v_key] = veh[1][tc.VAR_SPEED]
            self.output[d_key] = d1
        else:
            self.output[v_key] = 0
    ...
    def find_forward_vehs(self, sort_vehicles):
        ...
        if self.output['L'] == 0:
            ...
            self.output['v52'] = 0
            self.output['d52'] = max_dist
            d5_found = 1
            d52_found = 1
            #iterate through sorted vehicle lists for forward vehicles
            while idx < self.num_vehs and (not d1_found or not d3_found or not d12_found or not d32_found):
                ...
                #update left lane forward vehicle info
                    ...
                    d3_found = 1
                #update left lane 2nd forward vehicle info
                elif not d12_found and veh_lane == "Lane_0":
                    self.target_assignment(veh, 'v12', 'd12')
                    d12_found = 1
                #update left lane 2nd forward vehicle info
                elif not d32_found and veh_lane == "Lane_1":
                    self.target_assignment(veh, 'v32', 'd32')
                    d32_found = 1
                idx+=1
            if not d1_found:
                self.output['v1'] = 0
            ...
            if not d12_found:
                self.output['v12'] = 0
                self.output['d12'] = max_dist
            if not d32_found:
                self.output['v32'] = 0
                self.output['d32'] = max_dist
        #if Auto is in lane 1
        elif self.output['L'] == 1:
            #iterate through sorted vehicle lists for forward vehicles
            while idx < self.num_vehs and (not d1_found or not d3_found or not d5_found or not d12_found or not d32_found or not d52_found):
                veh = list(sort_vehicles.items())[idx]
                veh_lane = veh[1][tc.VAR_LANE_ID]
                #update in lane forward vehicle info
                ...
                #update right lane forward vehcile info
                    ...
                    d5_found = 1
                #update in lane 2nd forward vehicle info
                elif not d12_found and veh_lane == "Lane_1":
                    self.target_assignment(veh, 'v12', 'd12')
                    d12_found = 1
                #update left lane 2nd forward vehicle info
                elif not d32_found and veh_lane == "Lane_2":
                    self.target_assignment(veh, 'v32', 'd32')
                    d32_found = 1
                #update right lane 2nd forward vehcile info
                elif not d52_found and veh_lane == "Lane_0":
                    self.target_assignment(veh, 'v52', 'd52')
                    d52_found = 1
                idx+=1
            if not d1_found:
                self.output['v1'] = 0
            ...
            d32_found = 1
            #iterate through sorted vehicle lists for forward vehicles
            while idx < self.num_vehs and (not d1_found or not d5_found or not d12_found or not d52_found):
                veh = list(sort_vehicles.items())[idx]
                veh_lane = veh[1][tc.VAR_LANE_ID]
                #update in lane forward vehicle info
                ...
                #update right lane forward vehcile info
                    ...
                    d5_found = 1
                #update in lane 2nd forward vehicle info
                elif not d12_found and veh_lane == "Lane_2":
                    self.target_assignment(veh, 'v12', 'd12')
                    d12_found = 1
                #update right lane 2nd forward vehcile info
                elif not d52_found and veh_lane == "Lane_1":
                    self.target_assignment(veh, 'v52', 'd52')
                    d52_found = 1
                idx+=1
            if not d1_found:
...
#        #positive reward for being in the overtaking lane when a vhicle in front at close distance
            ...
            reward += r_lanechange - self.output['d5']/8
